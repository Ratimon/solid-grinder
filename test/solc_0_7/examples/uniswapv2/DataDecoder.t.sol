...
import {AddressTable} from "@main/solc_0_7/AddressTable.sol";

import {UniswapV2Router02_Encoder} from "@main/solc_0_7/examples/uniswapv2/encoder/UniswapV2Router02_Encoder.g.sol";
import {Mock_Decoder} from "@test/solc_0_7/examples/uniswapv2/mock/Mock_Decoder.sol";
...
contract UniswapV2Router02_DataDecoderTest is Test {
    ...
    function setUp() public {
        ...
        encoder = new UniswapV2Router02_Encoder(IAddressTable(address(table)));
        decoder = new Mock_Decoder(IAddressTable(address(table)));
    ...
    function test_constructor() external {
        ...
        assertEq(uint256(encoder.packedBits(0, 0)), 24);
        assertEq(uint256(encoder.packedBits(0, 1)), 24);
        assertEq(uint256(encoder.packedBits(0, 2)), 96);
        assertEq(uint256(encoder.packedBits(0, 3)), 96);

        assertEq(uint256(encoder.packedBits(1, 0)), 96);
        assertEq(uint256(encoder.packedBits(1, 1)), 96);
        assertEq(uint256(encoder.packedBits(1, 2)), 24);
        assertEq(uint256(encoder.packedBits(1, 3)), 40);
